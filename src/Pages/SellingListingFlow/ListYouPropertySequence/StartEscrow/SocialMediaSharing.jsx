...
function Row({ children, onClick }) {
  return (
    <div className="flex items-center justify-between py-4 px-4 border-b last:border-b-0 cursor-pointer" onClick={onClick}>
  ...
  return (
    <div className="mt-6">
      ...
      <div className="bg-white border rounded shadow-sm">
        {items.map((it, idx) => (
          <div key={idx}>
            <Row onClick={() => { /* placeholder */ }}>{it}</Row>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="mt-10">
        ...
        <div className="bg-white rounded shadow-sm border p-6">
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-500 mt-2">Your Listing Agent Requires Some Documents Like The Listing Agreement, Agency Disclosure, And Property Details Input Forms. The Sooner You Complete This The Quicker Your Listing Will Be Online</p>
          </div>

          <div className="divide-y">
            {mlsItems.map((it, idx) => (
              <div key={idx} className="py-3 flex items-center justify-between text-sm text-gray-700">
                <div>{it}</div>
                <div className="text-gray-400">▾</div>
              </div>
            ))}
