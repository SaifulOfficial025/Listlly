...
function Sidebar({ open, onClose }) {
  ...
  <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-56 flex-col justify-between bg-gradient-to-b from-[#3f7cf2] to-[#0e2b57] text-white">
        <div>
          ...
          <nav className="mt-4 space-y-1">
            ...
            <NavLink to="/vendor_dashboard/photography_request" className={() => `block w-full pl-6 py-3 text-sm ${isPhotographyActive ? 'bg-white text-[#1641ff] font-semibold' : 'text-white/90 hover:opacity-100'}`}>
              Photography Request
            </NavLink>
    </nav>
        </div>
  ...
  <aside className={`absolute left-0 top-0 bottom-0 w-64 max-w-full sm:w-64 bg-gradient-to-b from-[#3f7cf2] to-[#0e2b57] text-white transform transition-transform ${open ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="mt-4 space-y-1 px-4">
            <NavLink to="/dashboard" end onClick={onClose} className={({isActive}) => `block w-full pl-2 py-3 text-sm ${isActive ? 'bg-white text-[#1641ff] font-semibold' : 'text-white/90 hover:opacity-100'}`}> 
              Dashboard
            </NavLink>
            <NavLink to="/dashboard/photography_request" onClick={onClose} className={({isActive}) => `block w-full pl-2 py-3 text-sm ${isActive ? 'bg-white text-[#1641ff] font-semibold' : 'text-white/90 hover:opacity-100'}`}> 
              Photography Request
            </NavLink>
            ...
            <NavLink to="/dashboard/saved_properties" onClick={onClose} className={({isActive}) => `block w-full pl-2 py-3 text-sm ${isActive ? 'bg-white text-[#1641ff] font-semibold' : 'text-white/90 hover:opacity-100'}`}>Saved Properties</NavLink>
            <NavLink to="/dashboard/how_to_videos" onClick={onClose} className={({isActive}) => `block w-full pl-2 py-3 text-sm ${isActive ? 'bg-white text-[#1641ff] font-semibold' : 'text-white/90 hover:opacity-100'}`}>How To Videos</NavLink>
          </nav>

          {/* Contact info for mobile */}
          <div className="px-4 pb-6 pt-8 text-xs">
            <div className="text-white/90 font-semibold mb-2">Contact US</div>
            <div className="text-white/80 text-[11px] mb-3">For Proper Guidelines And Further Support</div>
